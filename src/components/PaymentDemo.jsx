  ...
  const paymentMethods = [
    {
      ...
      description: "Accept USDC, and other major stablecoins",
      icon: Wallet,
    ...
    <section className="w-full bg-gradient-to-b from-white to-gray-50 py-20 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className={`text-center mb-16 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <h2 className="text-4xl md:text-5xl font-extrabold text-black tracking-tight md:mb-4">
            See It In Action
          </h2>
        </div>

        <div className="grid lg:grid-cols-2 gap-6 md:gap-12 items-center">
          {/* Cart Screenshot - first on mobile */}
          <div className={`relative transition-all duration-1000 delay-300 order-1 lg:order-2 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
            ...
            <div className="flex justify-center mt-6 space-x-2">
              {paymentMethods.map((_, index) => (
                <button
                  key={index}
                  className={`w-3 h-3 rounded-full transition-all duration-300 ${activeMethod === index ? 'bg-gradient-to-r from-purple-500 to-pink-500 scale-125' : 'bg-gray-300 hover:bg-gray-400'}`}
          ...
          <div className="space-y-6 order-2 lg:order-1">
            {paymentMethods.map((method, index) => {
              ...
              return (
                <div
                  key={method.id}
                  className={`group relative p-6 rounded-2xl border-2 transition-all duration-500 cursor-pointer transform hover:scale-105 ${activeMethod === index
                ...
                >
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-gray-600 text-xs md:text-sm leading-relaxed">{method.description}</p>
                    </div>
                    {activeMethod === index && (
            ...
            {/* Features Badge */}
            <div className={`mt-8 transition-all duration-1000 delay-700 flex justify-center ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
